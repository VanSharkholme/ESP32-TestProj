...
void Channel_Struct_Clear(UI_Channel *channel)
{
    channel->pPlan = NULL;
    channel->timer.remaining_seconds = 0;
    channel->timer.state = UI_TIMER_STATE_UNINIT;
...
void clear_all_channels()
{
    ...
    Channel_Struct_Clear(&ChannelA);
    Channel_Struct_Clear(&ChannelB);
    Channel_Struct_Clear(&ChannelC);
    Channel_Struct_Clear(&ChannelD);
...
void set_battery_level(uint8_t soc)
{
    ...
    if (scr == charging_scr)
    {
        lv_obj_t *charging_ring = lv_obj_get_child(scr, 0);
